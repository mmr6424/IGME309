...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Rose Rushton - mmr6424@rit.edu";
...
void Application::Display(void)
{
	...
	case 2:
		...
		break;
	case 3:
		if (m_pCamera->GetPosition() != (AXIS_Z * 30.0f) || m_pCamera->GetUp() != (AXIS_Z * -1.0f)) {
			m_pCamera->ResetCamera();
			m_pCamera->SetPositionTargetAndUpward(AXIS_X * 30.0f, m_pCamera->GetTarget(), AXIS_Z * -1.0f);
		}
		break;
	case 4:
		if (m_pCamera->GetPosition() != (AXIS_Z * -13.0f) || m_pCamera->GetUp() != AXIS_Y) {
			m_pCamera->ResetCamera();
			m_pCamera->SetPositionTargetAndUpward(AXIS_Z * -13.0f, m_pCamera->GetTarget(), AXIS_Y);
		}
		break;
	case 5:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(AXIS_Z * -13.0f);
		m_pCamera->SetNearFar(vector2(6.0f, 13.0f));
		break;
	case 6:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(AXIS_Z * -13.0f);
		m_pCamera->SetNearFar(vector2(1.0f, 8.0f));
		break;
	case 7:
		if (m_pCamera->GetPosition() != vector3(0.0f, 0.0f, 10.0f) || m_pCamera->GetUp() != (AXIS_Y * -1.0f)) {
			m_pCamera->ResetCamera();
